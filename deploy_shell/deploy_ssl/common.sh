...
validate_project_name() {
  local value="${1:-}"
  [[ -n "$value" ]] || die "PROJECT_NAME 不能为空，请在 deploy_config.sh 中定义。命名规范：xxx_ssl"
  if [[ ! "$value" =~ ^[a-z0-9]+(_[a-z0-9]+)*_ssl$ ]]; then
    die "PROJECT_NAME 不符合命名规范: ${value}。仅支持下划线命名，示例：example_ssl"
  fi
}
