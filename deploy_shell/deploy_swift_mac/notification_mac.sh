...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      echo "[ERROR] 未知参数: $1" >&2
      echo "[ERROR] 用法: notification_mac.sh --config <deploy_config.sh绝对路径> --version <版本号> --version_code <构建号> --branch_name <分支> --build_id <构建ID> --artifact_path <产物路径> --channel <打包渠道> --build_status <success|failure> --environment <环境> [--error <message>]" >&2
      exit 1
      ;;
  esac
done
...
notification_init_content CONTENT "$BUILD_STATUS" "macos" "build" "$PROJECT_NAME"
...
notification_append_emoji_line CONTENT "💻" "构建平台" "macOS"
