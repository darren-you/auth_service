...
ARTIFACT_LOCAL_PATH="${LOCAL_NGINX_CONF:-$PROJECT_ROOT/nginx.conf}"
DEPLOY_TARGET="${DEPLOY_TARGET:-${DEPLOY_USER:-unknown}@${DEPLOY_HOST:-unknown}:${DEPLOY_PORT:-22}}"
DEPLOY_ARTIFACT_REMOTE="${DEPLOY_ARTIFACT_REMOTE:-${REMOTE_NGINX_CONF:-unknown}}"

if [[ "$STAGE" == "deploy" ]]; then
  EVENT_TIME="$DEPLOY_TIME"
fi

notification_init_content CONTENT "$STATUS" "nginx" "$STAGE" "$PROJECT_NAME"
